import React, {useState} from 'react';
import {
  StyleSheet,
  View,
  Dimensions,
  TouchableWithoutFeedback,
} from 'react-native';
import Animated, {
  scrollTo,
  useAnimatedStyle,
  withTiming,
  useAnimatedScrollHandler,
  useAnimatedRef,
  useSharedValue,
} from 'react-native-reanimated';

const windowWidth = Dimensions.get('window').width;

export default function CategoryBar({
  listRef,
  listCategoryLayout,
  categoryBarList,
}) {
  const [categoryBarLayout, setCategoryBarLayout] = useState([]);
  const categoryBarRef = useAnimatedRef();

  const selectedCategory = useSharedValue({
    x: 15,
    width: 0,
    index: 0,
  });

  function handleSelectCategory(index) {
    listRef.current.scrollTo({
      x: 0,
      y: listCategoryLayout[index].y,
      animated: true,
    });

    selectedCategory.value = {
      x: categoryBarLayout[index].x,
      width: categoryBarLayout[index].width,
      index: index,
    };
    categoryBarRef.current.scrollTo({
      y: 0,
      x:
        categoryBarLayout[index].x -
        (windowWidth / 2 - categoryBarLayout[index].width / 2),
      animated: true,
    });
  }

  function scrollToNearestItem(minDistance) {
    'worklet';

    var toScroll;
    const center = minDistance + windowWidth / 2;

    const closestItem = categoryBarLayout.reduce((a, b) => {
      return Math.abs(b.x - center) < Math.abs(a.x + a.width - center) ? b : a;
    });

    toScroll = closestItem.x - (windowWidth / 2 - closestItem.width / 2);

    scrollTo(categoryBarRef, toScroll, 0, true);
  }

  const horizontalScrollHandler = useAnimatedScrollHandler({
    onEndDrag: e => {
      if (e.velocity.x === 0) {
        scrollToNearestItem(e.contentOffset.x);
      }
    },

    onMomentumEnd: e => {
      if (
        e.contentOffset.x !== 0 &&
        e.contentOffset.x !== e.contentSize.width - windowWidth
      ) {
        scrollToNearestItem(e.contentOffset.x);
      }
    },
  });

  const selectedCategoryStyle = useAnimatedStyle(() => {
    if (
      selectedCategory.value.width === 0 &&
      categoryBarLayout.length === categoryBarList.length
    ) {
      selectedCategory.value = categoryBarLayout[0];
    }

    return {
      position: 'absolute',
      alignSelf: 'center',
      backgroundColor: 'rgba(168, 238, 144, 0.3)',
      height: 35,
      borderRadius: selectedCategory.value.width / 3,
      width: withTiming(selectedCategory.value.width, {duration: 200}),
      transform: [
        {
          translateX: withTiming(selectedCategory.value.x, {
            duration: 200,
          }),
        },
      ],
    };
  }, [selectedCategory.value, categoryBarLayout]);

  return (
    <Animated.ScrollView
      ref={categoryBarRef}
      showsHorizontalScrollIndicator={false}
      onScroll={horizontalScrollHandler}
      horizontal={true}
      scrollEventThrottle={16}>
      {categoryBarList.map((item, index) => {
        const selectedCategoryTextStyle = useAnimatedStyle(() => {
          return {
            fontSize: 18,
            color: selectedCategory.value.index === index ? 'green' : 'black',
          };
        });

        return (
          <TouchableWithoutFeedback
            onPress={() => handleSelectCategory(index)}
            key={index}>
            <View
              style={styles.categoryBar__option}
              onLayout={e => {
                if (categoryBarLayout.length <= categoryBarList.length) {
                  setCategoryBarLayout(
                    [
                      ...categoryBarLayout,
                      {...e.nativeEvent.layout, index: index},
                    ].sort((a, b) => (a.x > b.x ? 1 : -1)),
                  );
                }
              }}>
              <Animated.Text style={selectedCategoryTextStyle}>
                {item}
              </Animated.Text>
            </View>
          </TouchableWithoutFeedback>
        );
      })}
      <Animated.View style={selectedCategoryStyle} />
    </Animated.ScrollView>
  );
}

const styles = StyleSheet.create({
  categoryBar__option: {
    alignSelf: 'center',
    justifyContent: 'center',
    alignItems: 'center',
    height: '100%',
    height: 35,
    borderRadius: 100,
    marginLeft: 15,
    marginRight: 15,
    paddingLeft: 15,
    paddingRight: 15,
  },
});
